#!/bin/sh
mkdir -p /opt/ping
curl -sSL https://api.gcdn.co/cdn/public-ip-list > /opt/ping/temp_ip.txt
...
echo 'del D:\/Tools\/Ping\/ping6.log' >> /opt/ping/ping.bat
sed "s/^/ping -n 1 &/g" /opt/ping/ip6s.txt >>  /opt/ping/ping6.bat
sed -i "s/$/& >> D:\/Tools\/Ping\/ping6.log/g" /opt/ping/ping6.bat
